		...
		<div class="row">
			<div class="ibox-content">
				<table
					...
					<tbody>
							<c:forEach items="${customerLoss}" var="c">
								<tr>
									<td>${c.id}</td>
									<td>${c.cusName}</td>
									<td>${c.cusManager}</td>
									<td><fmt:formatDate value="${c.lastOrderTime}" type="date" dateStyle="default"/></td>
									<td><fmt:formatDate value="${c.confirmLossTime}" type="date" dateStyle="default"/></td>
									<td>
									<c:if test="${c.state==1}">确认流失</c:if>
									<c:if test="${c.state==0}">暂缓流失</c:if>
									</td>
									<td>${c.lossReason}</td>
									<td>
									<c:if test="${c.state==1}">客户确认流失</c:if>
									<c:if test="${c.state==0}"><a href="../reprieve/goreprieve.controller?lossId=${c.id}">暂缓流失</a></c:if>
									</td>
								</tr>
							</c:forEach>
							</tbody>
